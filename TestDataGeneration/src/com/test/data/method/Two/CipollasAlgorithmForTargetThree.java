...
public class CipollasAlgorithmForTargetThree implements CipollasAlgorithm{
    ...
    private static Triple c(String ns, String ps) {
        ...
        //target 3 approach level 1
        //target 3 branchDistance:
        //if -nn.compareTo(BigInteger.ZERO) < 0 then 0 else -nn.compareTo(BigInteger.ZERO)+penaltyConstK
        approachLevel = 1;
        if((-nn.compareTo(BigInteger.ZERO)) < 0 ){
            branchDistance = 0;
        } else {
            branchDistance = (-nn.compareTo(BigInteger.ZERO)) + penaltyConst;
        }
        while (nn.compareTo(BigInteger.ZERO) > 0) {
            //target 3 approach level 0
            //target 3 branchDistance:
            //if abs(nn.and(BigInteger.ONE)-BigInteger.ONE) = 0 then 0 else
            // abs(nn.and(BigInteger.ONE)-BigInteger.ONE)+penaltyConstK
            approachLevel = 0;
            if((nn.and(BigInteger.ONE).subtract(BigInteger.ONE)).abs().equals(BigInteger.ZERO)){
                branchDistance = 0;
            } else {
                branchDistance = (nn.and(BigInteger.ONE).subtract(BigInteger.ONE)).abs().doubleValue() + penaltyConst;
            }
    ...
    public double cipollasAlgorithmFitness(Individual individual) {
        CipollasAlgorithmForTargetThree.c(individual.getNs(),individual.getPs());
