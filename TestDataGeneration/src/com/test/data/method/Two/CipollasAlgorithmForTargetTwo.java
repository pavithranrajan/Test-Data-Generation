...
public class CipollasAlgorithmForTargetTwo implements CipollasAlgorithm{
    ...
    private static Triple c(String ns, String ps) {
        ...
        BigInteger omega2;
        //target 2 approach level 1
        //target 2 branchDistance:
        //if true then 0 else penaltyConstK
        approachLevel = 1;
        if(true){
            branchDistance = 0;
        }else {
        ...
        while (true) {
            ...
            //target 2 approach level 0
            //target 2 branchDistance:
            //if abs(ls.apply(n) - BigInteger.ONE) == 0 then 0 else abs(ls.apply(n) - BigInteger.ONE)+penaltyConstK
            approachLevel = 0;
            if((ls.apply(omega2).subtract(p.subtract(BigInteger.ONE))).abs().equals(BigInteger.ZERO)){
                branchDistance = 0;
            } else {
                branchDistance = (ls.apply(omega2).subtract(p.subtract(BigInteger.ONE))).abs().add(BigInteger.valueOf(penaltyConst)).doubleValue();
            }
    ...
    public double cipollasAlgorithmFitness(Individual individual) {
        CipollasAlgorithmForTargetTwo.c(individual.getNs(),individual.getPs());
