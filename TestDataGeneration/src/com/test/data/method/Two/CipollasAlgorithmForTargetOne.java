...
import com.test.data.method.One.JacobiSymbolForTargetOne;

import java.math.BigInteger;
import java.util.function.BiFunction;
import java.util.function.Function;

public class CipollasAlgorithmForTargetOne implements CipollasAlgorithm{
    ...
    private static Triple c(String ns, String ps) {
        ...
        //target 1 approach level 0
        //target 1 branchDistance:
        //if abs(ls.apply(n) - BigInteger.ONE) != 0 then 0 else penaltyConstK
        approachLevel = 0;
        if(!(ls.apply(n).subtract(BigInteger.ONE)).abs().equals(BigInteger.ZERO)){
            branchDistance = 0;
        } else{
    ...
    public double cipollasAlgorithmFitness(Individual individual) {
        CipollasAlgorithmForTargetOne.c(individual.getNs(),individual.getPs());
