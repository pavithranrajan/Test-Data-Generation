...
public class CipollasAlgorithmForTargetFive implements CipollasAlgorithm{
    ...
    private static Triple c(String ns, String ps) {
        ...
        //target 5 approach level 0
        //target 5 branchDistance:
        //if abs(r.x.multiply(r.x).mod(p) - n) != 0 then 0 else penaltyConstK
        // Step 5, check x * x = n
        approachLevel = 0;
        if(!(r.x.multiply(r.x).mod(p).subtract(n)).abs().equals(BigInteger.ZERO)){
            branchDistance = 0;
        } else {
    ...
    public double cipollasAlgorithmFitness(Individual individual) {
        CipollasAlgorithmForTargetFive.c(individual.getNs(),individual.getPs());
